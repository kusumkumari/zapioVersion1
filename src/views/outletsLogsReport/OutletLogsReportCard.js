...
import OutletLogsReportList from "./OutletLogsReportList";
...
class OutletLogsReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  handleChangeStart = date => {
    ...
    this.listOutletLogsReport(moment(date).format("YYYY-MM-DD H:mm:ss"), this.state.end_date, this.state.outlets);
  };
  ...
  handleChangeEnd = date => {
    ...
    this.listOutletLogsReport(this.state.start_date, moment(date).format("YYYY-MM-DD H:mm:ss"), this.state.outlets);
  };
  handleChangeOutlet = (e) => {
    ...
    this.listOutletLogsReport(this.state.start_date, this.state.end_date, outletArray);
  }
  selectAll = () => {
  ...
  unSelectAll = () => {
    ...
    this.listOutletLogsReport(this.state.start_date, this.state.end_date, []);
    // this.setState({ selectedOutlet: [...this.state.outlet.id] })
  }
  componentDidMount() {
    this.listOutletLogsReport(
  ...
  listOutletLogsReport = (startDate, endDate, outlet) => {
    ...
    lstOutletLogsAPI(
      ...
      apiResponse => {
        ...
        if (apiResponse.response.data.status == true) {
          this.setState(
            {
              data: apiResponse.response.data.logdata,
              dataLength: apiResponse.response.data.logdata.length,
      ...
      <Fragment>
        <Row>
          <Colxx xxs="12">
            ...
            &nbsp;
            <Breadcrumb
              heading="report.outletlogs-report"
...
export default injectIntl(OutletLogsReport);
