...
import AddOnReportList from "./AddOnReportList";
...
class AddOnReport extends Component {
  ...
  handleChangeStart = date => {
    ...
    this.listAddonReport(moment(date).format("YYYY-MM-DD H:mm:ss"), this.state.end_date, this.state.outlets);
  };
  ...
  handleChangeEnd = date => {
    ...
    this.listAddonReport(this.state.start_date, moment(date).format("YYYY-MM-DD H:mm:ss"), this.state.outlets);
  };
  ...
  handleChangeOutlet = (e) => {
    ...
    this.listAddonReport(this.state.start_date, this.state.end_date, outletArray);
  }
  selectAll = () => {
  ...
  unSelectAll = () => {
    ...
    this.listAddonReport(this.state.start_date, this.state.end_date, []);
    // this.setState({ selectedOutlet: [...this.state.outlet.id] })
  }
  componentDidMount() {
    this.listAddonReport(
  ...
  listAddonReport = (startDate, endDate, outlet) => {
            ...
            <Card className="rounded-sm">
              <CardBody className="d-flex align-items-center justify-content-center px-1 py-3">
                ...
                {this.state.dataLength > 0 ?
                  <a
                    href={API_BASE_URL + "/addon/report/csv/?start_date=" + this.state.start_date + "&end_date=" + this.state.end_date + "&cid=" + companyId1 + "&token=" + localStorage.getItem("token") + "&outlet_id=" + this.state.outlets}
...
export default injectIntl(AddOnReport);
