...
class AccountCard extends Component {
  ...
  changepasswordHandler = () => {
    ...
    changepasswordAPI({
      password: oldpassword, new_pwd: newpassword,
      c_pwd: confirmpassword, user_type: user_type
    }, ({ response }) => {
      if (response.data.success == true) {
        Notification(1, response.data.message, "Change Password Success")
      }
      if (response.data.oldpass == false) {
        Notification(0, response.data.message, "Change Password Error")
      }
      else {
    ...
    return (
      <Fragment>
        <Row>
          ...
          <Colxx xxs="6">
            <div id="form"></div>
            <i className="iconsminds-security-settings text-primary" style={{ fontSize: "x-large" }} />&nbsp;
